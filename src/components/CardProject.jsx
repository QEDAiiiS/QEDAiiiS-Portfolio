...
const CardProject = ({ src, name, link }) => {
  return (
    ...
    className="flex justify-center items-center mb-6 hover:scale-y-105 duration-500 rounded-2xl p-4 border border-stone-700">
      <figure className=" group rounded-[4px] relative [transform-style:preserve-3d] will-change-transform
       transition-transform duration-500 hover:[transform:translateZ(5px)_rotateX(-5deg)_rotateY(20deg)] ">
        <img src={`./imgs/${src}`} className=" hover:scale-110 duration-500" />


        <div className=" flex justify-between mt-3 group-hover:[transform:translateZ(50px)] duration-500 items-center">
          
        <figcaption className="  text-white  font-bold text-2xl [font-family:monospace]
          ">
          {name}
        </figcaption>
                 <a
